...
def test_no_pressure_proposes_no_defense() -> None:
    ...
    assert [item.owner for item in proposals] == [map_control.OWNER]
...
def test_an_air_attack_is_answered_only_by_units_that_shoot_up() -> None:
    ...
    assert result.owner_of(3) == map_control.OWNER
...
def test_defense_outranks_the_core_army_and_hands_units_back_when_the_attack_ends() -> None:
    ...
    assert set(dict(after.owners).values()) == {map_control.OWNER}
...
def test_the_units_defending_stay_on_the_attack_when_its_lowest_tag_dies() -> None:
    ...
    assert second.owner_of(4) == map_control.OWNER
