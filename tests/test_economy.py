...
def asking(
    planner: economy.Economy,
    *,
    time: float,
    count: int,
    economy_share: float,
    objective: Objective = Objective.BUILD_ADVANTAGE,
    workers: int = 83,
    sites: int = 9,
):
    """One frame of `planner`, with `strategy.economy` set to `economy_share`."""

    frame = attention(time=time, bases=bases(count), workers=workers, map_view=map_with(sites))
    strategy = StrategyModel().decide(frame, AwarenessModel().infer(frame))
    return planner.plan(frame, replace(strategy, objective=objective, economy=economy_share))


def test_an_ask_for_a_base_survives_a_dip_in_the_economy_preference() -> None:
    # bench/base3/001: the plan asked for a sixth base at 495.0 s, stopped
    # asking at 517.9 s when strategy.economy fell from 0.69 to 0.46 with
    # nothing built, asked again at 537.9 s, and the base started at 564.6 s.
    planner = economy.Economy()

    asked = asking(planner, time=495.0, count=5, economy_share=0.69)
    dipped = asking(planner, time=517.9, count=5, economy_share=0.46)
    later = asking(planner, time=533.7, count=5, economy_share=0.45)

    assert (asked.expand, asked.bases, asked.reason) == (True, 6, "mineral_lines_saturated")
    assert (dipped.expand, dipped.bases) == (True, 6)
    assert dipped.reason == "expansion_asked_for"
    assert dict(dipped.inputs)["expansion_held"] == 1.0
    assert (later.expand, later.bases) == (True, 6)

    # The stateless plan of the frame is what dropped it.
    frame = attention(time=517.9, bases=bases(5), workers=83, map_view=map_with(9))
    strategy = StrategyModel().decide(frame, AwarenessModel().infer(frame))
    assert not economy.plan(frame, replace(strategy, economy=0.46)).expand


def test_the_ask_ends_once_the_townhall_is_there() -> None:
    planner = economy.Economy()
    asking(planner, time=495.0, count=5, economy_share=0.69)

    # attention.bases counts a Command Center still being built.
    arrived = asking(planner, time=564.6, count=6, economy_share=0.46)

    assert (arrived.expand, arrived.bases, arrived.reason) == (False, 6, "build_economy")
    assert dict(arrived.inputs)["expansion_held"] == 0.0


def test_a_threat_ends_the_ask_as_it_ends_the_upgrades() -> None:
    planner = economy.Economy()
    asking(planner, time=495.0, count=5, economy_share=0.69)

    stabilizing = asking(
        planner, time=500.0, count=5, economy_share=0.46, objective=Objective.STABILIZE
    )
    after = asking(planner, time=505.0, count=5, economy_share=0.46)

    assert stabilizing.expand is False
    # The ask is gone, not merely paused: it is decided again on its own terms.
    assert (after.expand, after.reason) == (False, "build_economy")


def test_an_ask_that_never_becomes_a_townhall_expires() -> None:
    planner = economy.Economy()
    asking(planner, time=495.0, count=5, economy_share=0.69)

    commit = economy.DEFAULTS.expand_commit
    inside = asking(planner, time=495.0 + commit - 0.1, count=5, economy_share=0.46)
    expired = asking(planner, time=495.0 + commit, count=5, economy_share=0.46)

    assert inside.expand is True
    assert (expired.expand, expired.reason) == (False, "build_economy")


def test_the_ask_is_never_started_below_the_threshold() -> None:
    # What the fog slice protects: a bot behind in army does not start a base.
    planner = economy.Economy()

    first = asking(planner, time=495.0, count=5, economy_share=0.46)
    second = asking(planner, time=500.0, count=5, economy_share=0.46)

    assert (first.expand, second.expand) == (False, False)
    assert dict(second.inputs)["expansion_held"] == 0.0


def test_the_ask_ends_when_the_map_runs_out_or_the_lines_empty() -> None:
    planner = economy.Economy()
    asking(planner, time=495.0, count=5, economy_share=0.69, sites=9)
    assert asking(planner, time=500.0, count=5, economy_share=0.46, workers=40).expand is False

    planner = economy.Economy()
    asking(planner, time=495.0, count=5, economy_share=0.69, sites=9)
    assert asking(planner, time=500.0, count=5, economy_share=0.46, sites=5).expand is False


@pytest.mark.parametrize("field, value", [("expand_commit", -1.0), ("expand_economy", 1.5)])
def test_a_config_outside_its_limits_is_refused(field: str, value: float) -> None:
    with pytest.raises(ValueError):
        economy.EconomyConfig(**{field: value})
