...
- Produção: o `ProductionController` fica depois do `SpawnController` no `MacroPlan` e só roda num frame em que
...
| `behavior.economy_planned` | behaviors | o plano muda | `active`, `workers`, `gas`, `bases`, `expand`, `freeflow`, `reason`, `composition[]`, `inputs` {`workers`, `bases`, `saturated_at`, `strategy_economy`, `upgrades_done`, `danger`}, `upgrades[]`, `orbitals`, `mules`, `interrupt_opening` |
