...
- Add-ons: `reactors` liga depois do opening e fora de STABILIZE (como os upgrades). A Barracks pronta, ociosa e
  sem add-on de menor tag recebe `BARRACKSREACTOR`, uma por frame, enquanto sobrarem mais de
...
| `behavior.economy_planned` | behaviors | o plano muda | `active`, `workers`, `gas`, `bases`, `expand`, `freeflow`, `reason`, `composition[]`, `inputs` {`workers`, `bases`, `saturated_at`, `strategy_economy`, `upgrades_done`, `danger`, `production_per_base`, `techlab_reserve`}, `upgrades[]`, `orbitals`, `mules`, `interrupt_opening`, `max_production`, `reactors`, `techlab_reserve` |
