...
- **Intel:** abrir, reabrir ou encerrar antes da saída é do planner, e é dele também a decisão de mandar
  ...
  deixa o SCV parado; um lugar inalcançável cede a vez depois de `surveil_step`. Só o fim da janela da
  opening encerra. A missão não registra nada: o que ela põe à vista, a Attention grava, e é isso que as
  fases leem.
...
| `opening_scout.phase_changed` | missions | o early scout abre, muda de fase ou termina | `mission_id`, `phase` (`REQUESTING`, `CHECK_NATURAL`, `ENTER_MAIN`, `CIRCLE_MAIN`, `RECHECK_NATURAL`, `CHECK_THIRD`, `SURVEIL`, `PROXY_SEARCH`, `COMPLETE`), `previous`, `since`, `reason` (em `SURVEIL`: `third_found`, `third_empty`, `third_window_over`, `no_third_known`), `status`, `target`, `proxy_search`, `inputs` {`set_out`, `proxy_ordered`, `proxy_index`, `proxy_points`, `watching`, `watchpoints`} |
