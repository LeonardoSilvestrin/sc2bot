...
- **Intel:** abrir, reabrir ou encerrar antes da saída é do planner, e é dele também a decisão de mandar
  ...
  janela do third vem da raça (`scout_window`). A missão não registra nada: o que ela põe à vista, a
  Attention grava, e é isso que as fases leem.
...
| `opening_scout.phase_changed` | missions | o early scout abre, muda de fase ou termina | `mission_id`, `phase` (`REQUESTING`, `CHECK_NATURAL`, `ENTER_MAIN`, `CIRCLE_MAIN`, `RECHECK_NATURAL`, `CHECK_THIRD`, `PROXY_SEARCH`, `COMPLETE`), `previous`, `since`, `reason`, `status`, `target`, `proxy_search`, `inputs` {`set_out`, `proxy_ordered`, `proxy_index`, `proxy_points`} |
...
- **Informação:** a Strategy ainda não consome a `OpeningBelief` (Attention, Awareness e Intel já a
  ...
  scouting depois do early game (o SCV sai uma vez, antes de 240 s), belief
