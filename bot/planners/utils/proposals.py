...
class PickPolicy(Protocol):
    """
    Planner-owned unit selection policy.
    Ego uses this as the anchor contract for deterministic tag selection.
    """
...
class UnitRequirement:
    ...
    def validate(self) -> None:
        ...
        if self.pick_policy is None:
            raise ValueError("UnitRequirement.pick_policy is required")
        if not hasattr(self.pick_policy, "allow") or not callable(getattr(self.pick_policy, "allow")):
            raise TypeError("UnitRequirement.pick_policy must provide callable allow(unit, *, bot, attention, now)")
        if not hasattr(self.pick_policy, "score") or not callable(getattr(self.pick_policy, "score")):
            raise TypeError("UnitRequirement.pick_policy must provide callable score(unit, *, bot, attention, now)")
        if not isinstance(getattr(self.pick_policy, "name", None), str) or not str(getattr(self.pick_policy, "name")).strip():
            raise TypeError("UnitRequirement.pick_policy.name must be a non-empty string")
        if not isinstance(self.required, bool):
            raise TypeError("UnitRequirement.required must be bool")
...
class Proposal:
    ...
    domain: str
    score: int
    reinforce_mission_id: Optional[str] = None
    ...
    def validate(self) -> None:
        ...
        if self.reinforce_mission_id is not None:
            if not isinstance(self.reinforce_mission_id, str):
                raise TypeError("Proposal.reinforce_mission_id must be str or None")
            if not self.reinforce_mission_id.strip():
                raise ValueError("Proposal.reinforce_mission_id must be non-empty when provided")
