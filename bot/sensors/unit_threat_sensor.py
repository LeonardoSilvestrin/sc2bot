...
def derive_unit_threat_snapshot(
    ...
    *,
    ...
    if not hasattr(bot, "mediator"):
        raise RuntimeError("missing_contract:mediator")
    enemy_units = getattr(bot.mediator, "get_all_enemy", None)
    if enemy_units is None:
        raise RuntimeError("missing_contract:mediator.get_all_enemy")
    if enemy_units.amount == 0:
    ...
    for mission in missions.ongoing:
        ...
        ground_threats_mission = enemy_near_mission.filter(lambda e: bool(getattr(e, "can_attack_ground", False)))
        can_win_val = _can_win_value(bot, live_units, ground_threats_mission)
        can_win = bool(int(can_win_val) >= 5)
        ...
        for unit in live_units:
            ...
            local_can_win = int(_can_win_value(bot, [unit], near))
            lose_pressure = max(0.0, float(5 - local_can_win)) * 1.8
            local_density = float(enemy_count_local) * 0.9
            hp_pressure = max(0.0, (1.0 - float(hp_frac)) * 3.0)
            danger = float(lose_pressure + local_density + hp_pressure)
            in_danger = bool(danger >= 6.0 or (local_can_win <= 2 and enemy_count_local > 0))
            if in_danger:
                units_in_danger += 1
            units_out.append(
                UnitThreatSnapshot(
        ...
        missions_out.append(
            MissionUnitThreatSnapshot(
                ...
                can_win_value=int(can_win_val),
                can_win_fight=bool(can_win),
            )
        )
