...
with its centre ahead of the Tank. With none ahead the Tank is hemmed in: born
into a pocket of production, cliff and doodads, the way toward its point is not
the way out (a Tank beside the Factory that made it and facing a cliff, from
7:00 to 20:00 of `game-20260919T164509532567Z`). Its blocker is then one whose
footprint or add-on comes within `hem_reach` of the Tank. One without an add-on
goes first -- lifting leaves the add-on behind -- then the one closest.
Command Centers never move.

One relocation at a time: lift the blocker, ordering it again every frame until
it flies -- the game refuses a lift queued behind what the structure still
trains, and Ares fills an idle structure again the next frame; once it flies,
wait until the Tank has covered `progress_distance` from where it was stuck (or
is gone, or `wait_timeout` passed); then land it on the nearest free production
site of the main or of another base we hold whose footprint keeps `lane` cells
from the Tank's way and from the ramp corridor. With no such site it stays in
the air and looks again every `land_retry` seconds, and after `return_after`
seconds it lands back where it was: a structure left flying trains nothing and
holds every other relocation back (a Factory flew 744 s until it died in that
game). A site not landed on within `land_timeout` is dropped for the next one.
Once a relocation ends no other starts for `global_cooldown` seconds, and a
structure is not lifted again for `structure_cooldown` seconds after its lift.
...
class RelocationConfig:
    ...
    # With none ahead, how close to the Tank's centre a footprint or its add-on
    # comes to hem it in: a Tank's radius, 0.875, and a little.
    hem_reach: float = 1.25
    ...
    # Seconds a lifted structure with nowhere else to land keeps flying before
    # it lands back where it was.
    return_after: float = 30.0
...
class Relocator:
    ...
    def plan(self, attention: AttentionState) -> RelocationStep:
        ...
        if self._active is not None:
            ...
            if self._active is not None and self._active.phase == LIFTING:
                lift = (self._active.structure,)
        ...
        stuck_events = [
            RelocationEvent(
                ...
                tank=tag,
                at=tanks[tag].position,
                inputs=self._stuck_inputs(attention, tanks[tag]),
            )
    ...
    def _stuck_inputs(
        self, attention: AttentionState, tank: UnitView
    ) -> tuple[tuple[str, float], ...]:
        track = self._tracks[tank.tag]
        inputs = (
            ("stuck_for", attention.time - track.since),
            ("moved", tank.position.distance_to(track.anchor)),
            ("destination_distance", tank.position.distance_to(track.destination)),
        )
        nearest = min(
            (
                _gap_to(tank.position, _footprint(structure))
                for structure in attention.own_structures
                if structure.type_id in MOBILE
            ),
            default=None,
        )
        if nearest is not None:
            inputs += (("nearest_production", nearest),)
        return inputs
    ...
    def _start(
        ...
        for tag in self._stuck(now):
            ...
            if chosen is None:
                continue
            blocker, gap, candidates, ahead = chosen
    ...
    def _blocker(
    ...
    ) -> tuple[UnitView, float, int, bool] | None:
        """The structure to lift for `tank`, how far it is from the Tank's way
        (hemmed in, from the Tank), how many could have been lifted, and
        whether it was ahead."""
        ...
        liftable = [
            ...
            and now - self._lifted_at.get(structure.tag, -math.inf) >= config.structure_cooldown
        ]
        candidates = self._ahead(tank, destination, liftable)
        ahead = bool(candidates)
        if not ahead:
            for structure in liftable:
                gap = _gap_to(tank.position, _footprint(structure))
                if gap <= config.hem_reach:
                    candidates.append((structure.has_add_on, gap, structure.tag, structure))
        if not candidates:
            return None
        _, gap, _, blocker = min(candidates, key=lambda item: item[:3])
        return blocker, gap, len(candidates), ahead

    def _ahead(
        self, tank: UnitView, destination: Point2, liftable: list[UnitView]
    ) -> list[tuple[bool, float, int, UnitView]]:
        """The structures on the Tank's next cells toward its point."""
    ...
    def _advance(
        ...
        if site == relocation.origin:
            reason = "back_to_origin"
        else:
            reason = "retry" if relocation.failed else "site_found"
        events.append(
            self._event(
                "relocating",
                reason,
    ...
    def _landing_site(
        ...
        corridor, and how many free sites there were. With none free
        `return_after` seconds into the landing, the origin, once nothing
        stands on it."""
        ...
        for site in _held_sites(attention):
        ...
        # The add-on it left behind, if any, is its own again.
        origin = _square(relocation.origin, PRODUCTION_HALF_SIDE)
        if attention.time - relocation.since >= self.config.return_after and not any(
            _overlap(origin, other) for other in taken
        ):
            return relocation.origin, 0
        return None, 0
...
def _held_sites(attention: AttentionState) -> set[Point2]:
    """The production sites of the main and of every other base we hold."""

    sites = set(attention.map.production_sites)
    for key, group in attention.map.base_production_sites:
        if any(key.distance_to(base.position) < 1.0 for base in attention.bases):
            sites.update(group)
    return sites


def _footprint(structure: UnitView) -> tuple[Box, ...]:
    """A production structure, and its add-on if it has one."""

    if structure.has_add_on:
        return _site_boxes(structure.position)
    return (_square(structure.position, PRODUCTION_HALF_SIDE),)


def _gap_to(point: Point2, boxes: tuple[Box, ...]) -> float:
    return min(_box_distance(point, box) for box in boxes)
