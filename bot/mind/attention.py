...
class MacroSnapshot:
    """
    Macro operational state (NOT resources, NOT worker allocation).
    Keep it focused so it doesn't overlap EconomySnapshot.
    """
    opening_done: bool
    bases_total: int
...
def derive_attention(bot, *, awareness: Awareness, threat: Threat, log=None) -> Attention:
    ...
    game_state = derive_game_state_snapshot(bot)
