...
def derive_mission_unit_threat_intel(
    ...
    *,
    ...
    for mission_id, ms in m_by_id.items():
        ...
        focus_target_pos = None
        nxt = awareness.mem.get(K("intel", "locations", "next_scan_target"), now=now, default={}) or {}
        if isinstance(nxt, dict):
            try:
                focus_target_pos = {"x": float(nxt.get("x", 0.0) or 0.0), "y": float(nxt.get("y", 0.0) or 0.0)}
            except Exception:
                focus_target_pos = None

        prev_state = awareness.mem.get(K("intel", "units", "mission", mission_id, "state"), now=now, default={}) or {}
        ...
        prev_tick = int(awareness.mem.get(K("intel", "units", "mission", mission_id, "danger_tick"), now=now, default=0) or 0)
        ...
        state = {
            ...
            "focus_target_pos": focus_target_pos,
            "updated_at": float(now),
        }
        if changed and min_interval_ok:
            changed_any = True
        if changed or min_interval_ok:
            awareness.mem.set(K("intel", "units", "mission", mission_id, "state"), value=state, now=now, ttl=float(cfg.ttl_s))
            awareness.mem.set(
                K("intel", "units", "mission", mission_id, "danger_tick"),
                value=int(danger_tick),
                now=now,
                ttl=float(cfg.ttl_s),
            )
            awareness.mem.set(
                K("intel", "units", "mission", mission_id, "risk_level"),
                value=str(risk_level),
                now=now,
                ttl=float(cfg.ttl_s),
            )
            awareness.mem.set(
                K("intel", "units", "mission", mission_id, "units_in_danger"),
                value=int(units_in_danger),
                now=now,
                ttl=float(cfg.ttl_s),
            )
            awareness.mem.set(
                K("intel", "units", "mission", mission_id, "retreat_recommended"),
                value=bool(retreat_recommended),
                now=now,
                ttl=float(cfg.ttl_s),
            )
            awareness.mem.set(
                K("intel", "units", "mission", mission_id, "reinforce_needed"),
                value=bool(reinforce_needed),
                now=now,
                ttl=float(cfg.ttl_s),
            )
            awareness.mem.set(
                K("intel", "units", "mission", mission_id, "engage_window_s"),
    ...
    awareness.mem.set(K("intel", "units", "missions", "summary"), value=list(summary), now=now, ttl=float(cfg.ttl_s))
    awareness.mem.set(K("intel", "units", "global", "worst_mission_id"), value=str(worst_id), now=now, ttl=float(cfg.ttl_s))
    awareness.mem.set(K("intel", "units", "global", "critical_missions_top3"), value=list(critical_top3), now=now, ttl=float(cfg.ttl_s))

    prev_ver = int(awareness.mem.get(K("intel", "units", "version"), now=now, default=0) or 0)
    ver = int(prev_ver + 1) if changed_any else int(prev_ver)
    awareness.mem.set(K("intel", "units", "version"), value=int(ver), now=now, ttl=None)
