...
def _rush_math_signals(*, now: float, eb, cfg: OpeningIntelConfig, workers_peak_seen: int = 0, enemy_race: str = "UNKNOWN") -> dict:
    """Compute rush evidence signals.

    Philosophy:
    - Structural rules fire only on *concrete observed evidence* (pool seen, 2rax seen, proxy seen).
    - Economic imbalance (spending more on production than expected) is a secondary signal,
      gated on having vision of the enemy.
    - Absence of natural only scores if we *have* seen the enemy main — blindness ≠ one-base.
    - The score drives SUSPECTED/CONFIRMED thresholds; hard_rush = any rule with confirm=True.
    """
    ...
    # --- We have vision of the enemy if we've seen structures or workers ---
    have_main_vision = bool(
        int(production_structures_visible) >= 1
        or int(workers_seen_now) >= 1
        or int(enemy_bases) >= 1
    )
    ...
    # 1. Timing rules: concrete structural evidence (pool, rax, gate timings).
    for rule in rules:
    ...
    # 2. Worker deficit: only counts when we've actually seen workers (have vision).
    #    If we've never seen a worker, the deficit is just fog-of-war noise.
    worker_deficit_score = 0.0
    if (
        have_main_vision
        and int(workers_seen_now) >= 1  # must have actually observed workers
        and worker_deficit >= int(cfg.worker_under_count_tolerance)
    ...
    # 3. One-base alert: only if we saw exactly 1 base (enemy_bases == 1 means we
    #    saw a townhall but no expansion). Zero = no vision.
    ...
    # 4. No-natural alert: only if we have main vision and natural is genuinely absent.
    #    Absence of vision != absence of expansion.
    if (
        have_main_vision
        and float(now) <= float(cfg.no_natural_alert_until_s)
        and not nat_on_ground
        and nat_prog <= 0.0
    ):
        score += float(cfg.no_natural_alert_score)

    # 5. Production stack: more production buildings than economically expected for the time.
    #    This catches "spending on army infrastructure instead of expansion/workers".
    #    Only meaningful when we have vision.
    if have_main_vision and not nat_on_ground and int(production_structures_visible) >= 3 and float(now) <= 300.0:
    ...
    # 6. Race-specific stack patterns (with vision, no natural).
    if have_main_vision and not nat_on_ground and float(now) <= 260.0:
        if race_u == "PROTOSS":
    ...
    # 7. Large visible army without natural (direct army spending signal).
    if have_main_vision and not nat_on_ground and float(now) <= 260.0 and int(visible_non_worker_total) >= 8:
    ...
    # --- Macro aggressor detection ---
    # Enemy expanded (has natural) but scouting showed heavy early aggression that faded.
    # This signals "fake rush → macro" — the right response is harass, not prolonged defense.
    is_macro_aggressor = bool(
        nat_on_ground
        and float(nat_prog) >= 0.50
        and int(enemy_bases) >= 2
        and worker_deficit <= int(cfg.worker_under_count_tolerance)
    )
...
def _evaluate_opening(
    *,
    ...
    have_main_vision = bool(rush_math.get("have_main_vision", False))
    is_macro_aggressor = bool(rush_math.get("is_macro_aggressor", False))
    ...
    elif is_macro_aggressor and near_bases <= 1 and not threatened:
        # Enemy faked aggression early, now expanding — signal harass opportunity.
        kind = "MACRO_AGGRESSOR"
        conf = 0.72
    ...
    # SUSPECTED: structural evidence confirmed by scout OR real combat contact.
    # Score alone during early game is NOT enough — avoids reacting to fog-of-war.
    is_suspected = bool(
        ...
        and (
            ...
            or int(near_bases) >= 2
            or bool(threatened)
            or (early and hard_rush)
            or (early and have_main_vision and rush_score >= float(cfg.rush_score_suspected))
        )
    )
    ...
    # Macro aggressor: enemy expanded after early pressure — end the rush state.
    rush_macro_end = bool(is_macro_aggressor and clear_for >= float(cfg.rush_end_clear_s))
    ...
    if is_confirmed:
        ...
        if rush_likely_end or rush_forced_end or rush_macro_end or (rush_state == "SUSPECTED" and rush_suspected_decay):
            rush_state = "ENDED"
        else:
            rush_state = "HOLDING"
    else:
    ...
    signals = {
        ...
        "rush_macro_end": bool(rush_macro_end),
        "have_main_vision": bool(have_main_vision),
        "is_macro_aggressor": bool(is_macro_aggressor),
    }
...
def derive_enemy_opening_intel(
    ...
    *,
    ...
    # Require that we actually scouted the main (reaper done or recently seen main),
    # or have hard structural evidence — pure "dispatched SCV + 4 units" was too eager.
    scout_no_natural_suspected = bool(
        ...
        and (
            ...
            or (
                bool(reaper_done or main_seen_recent)
                and (
                    int(production_structures_visible) >= 2
    ...
    # Do not override rush state based on "no natural" evidence when the enemy is
    # clearly in macro mode (expanded) — seeing 2+ bases cancels the no-natural signal.
    _enemy_macro_evident = bool(is_macro_aggressor or int(enemy_bases_visible) >= 2)
    _no_nat_override_ok = bool(not _enemy_macro_evident)
    if _no_nat_override_ok and (scout_no_natural_confirmed or no_natural_structural_confirmed):
    ...
    elif _no_nat_override_ok and scout_no_natural_suspected and rush_state_out in {"NONE", "ENDED"}:
    ...
    if _no_nat_override_ok and no_natural_structural_confirmed and (
    ...
    if since_confirmed <= float(cfg.rush_confirmed_min_hold_s) and rush_state_out in {"NONE", "ENDED", "SUSPECTED"} and not bool(_enemy_macro_evident):
    ...
    if (bool(scout_no_natural_confirmed) or bool(no_natural_structural_confirmed)) and bool(_no_nat_override_ok):
        last_seen_pressure_out = float(now)
    ...
    # Macro aggressor: enemy did early aggression but is now macro-ing.
    # Published so harass planner can exploit the opening instead of staying defensive.
    is_macro_aggressor_out = bool(signals.get("is_macro_aggressor", False))
    awareness.mem.set(
        K("enemy", "opening", "macro_aggressor"),
        value=bool(is_macro_aggressor_out),
        now=now,
        ttl=float(cfg.ttl_s),
    )
