...
def derive_map_control_intel(
    ...
    *,
    ...
    elif bool(nat_offsite) and bool(no_direct_pressure):
        if float(own_total_power) < max(4.0, float(cfg.min_total_power) - 2.0):
            defer_reason = "offsite_nat_army_too_small"
        elif float(own_main_power) < max(2.0, float(cfg.min_main_power) - 1.0):
            defer_reason = "offsite_nat_main_hold_too_thin"
        elif float(presence_nat_side_power) > min(float(cfg.max_enemy_presence_power_at_nat), 1.2):
            defer_reason = "offsite_nat_enemy_presence"
        elif float(nat_enemy_power) > min(float(cfg.max_enemy_power_at_nat), 1.2):
            defer_reason = "offsite_nat_enemy_controls_nat"
        else:
            should_secure = True
            defer_reason = "recover_offsite_nat"
    ...
    elif bool(delayed_natural_alarm):
        if float(clear_for) < float(cfg.enemy_macro_lead_clear_s):
            defer_reason = "delayed_nat_pressure_not_clear"
        elif float(own_total_power) < float(cfg.delayed_natural_min_total_power):
            defer_reason = "delayed_nat_army_too_small"
        elif float(own_main_power) < float(cfg.delayed_natural_min_main_power):
            defer_reason = "delayed_nat_main_hold_too_thin"
        elif float(presence_nat_side_power) > float(cfg.delayed_natural_max_enemy_presence_power_at_nat):
            defer_reason = "delayed_nat_enemy_presence"
        elif float(nat_enemy_power) > float(cfg.delayed_natural_max_enemy_power_at_nat):
            defer_reason = "delayed_nat_enemy_controls_nat"
        else:
            should_secure = True
            defer_reason = "delayed_nat_alarm_secure_nat"
    else:
    ...
    if bool(nat_taken):
        ...
        # Offsite naturals need a way to break the bunker/tank -> land deadlock.
        safe_to_land = bool(
            nat_clear_to_land
            and (
                fortified
