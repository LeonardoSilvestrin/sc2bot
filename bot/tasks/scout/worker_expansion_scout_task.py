...
class WorkerExpansionScoutTask(BaseTask):
    ...
    def _mark_route_attempt(self, *, now: float, status: str, label: str, target: Point2 | None) -> None:
        payload = {
            "last_t": float(now),
            "status": str(status),
            "label": str(label),
        ...
        self.awareness.mem.set(K("intel", "worker_scout", "last_route_t"), value=float(now), now=now, ttl=None)
        self.awareness.mem.set(K("intel", "worker_scout", "last_attempt"), value=payload, now=now, ttl=180.0)
    ...
    async def on_step(self, bot, tick: TaskTick, attention: Attention) -> TaskResult:
        ...
        if hp_frac <= float(self.retreat_hp_frac):
            label = self.labels[min(int(self._idx), len(self.labels) - 1)]
            target = self.route[min(int(self._idx), len(self.route) - 1)]
            self._mark_route_attempt(now=now, status="retreat_low_hp", label=label, target=target)
        ...
        if local_danger > 0:
            self._mark_route_attempt(now=now, status="retreat_danger", label=label, target=target)
            unit.move(self._retreat_target(bot))
        ...
        if float(unit.distance_to(target)) <= float(self.arrive_radius):
            ...
            self._idx += 1
            if self._idx >= len(self.route):
                self._mark_route_attempt(now=now, status="route_complete", label=label, target=target)
                unit.move(self._retreat_target(bot))
                self._done("route_complete")
                return TaskResult.done("route_complete")
            target = self.route[self._idx]
