...
class LandBaseTask(BaseTask):
    ...
    def _clear_landing_zone(self, bot, *, target_pos: Point2, landing_unit_tag: int) -> bool:
        ...
        fallback = self._point_from_payload(snapshot.get("staging"), fallback=None)
        if fallback is None:
            fallback = getattr(bot, "start_location", None)
        try:
            if fallback is not None and float(fallback.distance_to(target_pos)) < 7.0:
