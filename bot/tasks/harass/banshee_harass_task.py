...
class BansheeHarass(BaseTask):
    ...
    async def on_step(self, bot, tick: TaskTick, attention: Attention) -> TaskResult:
        ...
        if any(float(getattr(b, "health_percentage", 1.0) or 1.0) <= float(self.retreat_hp_frac) for b in banshees):
            self._phase = 2
        ...
        if self._phase == 1:
            for b in banshees:
                ...
                if len(anti_air) >= 3:
                    self._phase = 2
                    break
                target = self._closest_worker(b, local_enemies)
                if target is not None:
                    b.attack(target)
                else:
                    b.move(harass_target)
            if self._phase == 1:
