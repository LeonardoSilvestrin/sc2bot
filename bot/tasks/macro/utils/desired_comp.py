...
def desired_priority_unit_names(*, awareness, now: float) -> list[str]:
    return [str(u.name) for u in desired_priority_units(awareness=awareness, now=now)]
...
def desired_controller_dict_names(*, awareness, now: float) -> dict[str, dict[str, float | int]]:
    comp = desired_controller_dict(awareness=awareness, now=now)
    return {str(uid.name): dict(cfg) for uid, cfg in comp.items()}
