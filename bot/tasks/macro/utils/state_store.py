...
class MacroStateStore:
    ...
    def set_rush_active(self, *, now: float, active: bool, ttl: float = 30.0) -> None:
        self.awareness.mem.set(K("macro", "defense", "rush", "active"), value=bool(active), now=now, ttl=float(ttl))

    def set_rush_fortify_last(self, *, now: float, depots: int, bunkers: int) -> None:
        self.awareness.mem.set(K("macro", "defense", "rush", "fortify_last_t"), value=float(now), now=now, ttl=None)
        self.awareness.mem.set(
            K("macro", "defense", "rush", "fortify_last"),
