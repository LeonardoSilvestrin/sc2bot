...
class DefendBaseTask(BaseTask):
    ...
    def _height(bot, pos: Point2) -> float:
        if not DefendBaseTask._pathable(bot, pos):
            return -9999.0
        try:
    ...
    def _highground_anchor(self, bot, *, base_pos: Point2, mineral_center: Point2) -> Point2:
        ...
        fallback = wall_hint if self._pathable(bot, wall_hint) else (base_pos if is_main else (mineral_center if self._pathable(bot, mineral_center) else base_pos))
        best = fallback
        best_h = self._height(bot, best)
        for p in points:
            h = self._height(bot, p)
            if h > best_h:
                best = p
                best_h = h
        return best
    ...
    @staticmethod
    def _is_main_base(bot, *, base_pos: Point2) -> bool:
        try:
            return float(base_pos.distance_to(bot.start_location)) <= 10.0
        except Exception:
            return False

    def _defense_anchor(self, bot, *, base_pos: Point2, tank_anchor: Point2) -> Point2:
        if not self._is_main_base(bot, base_pos=base_pos):
            return base_pos
        wall_hint = self._wall_hint(bot, base_pos=base_pos)
        candidates = [tank_anchor, wall_hint]
        try:
            ramp = getattr(bot, "main_base_ramp", None)
            top = getattr(ramp, "top_center", None) if ramp is not None else None
            barracks_pos = getattr(ramp, "barracks_correct_placement", None) if ramp is not None else None
            ...
            if barracks_pos is not None:
                candidates.insert(1, barracks_pos)
        except Exception:
            pass
        for p in candidates:
            try:
                if bool(bot.in_pathing_grid(p)):
                    return p
            except Exception:
                return p
        return tank_anchor
