...
class SecureBaseTask(BaseTask):
    ...
    def _best_ramp_tank_anchor(self, bot) -> Point2:
        ...
        preferred_center = self.staging_pos.towards(self.hold_pos, 1.5)
        fallback = self._safe_anchor(bot, preferred=preferred_center, fallback=self.staging_pos)
        best = fallback
        best_score = -9999.0

        candidates = [preferred_center, self.staging_pos, self.base_pos.towards(self.staging_pos, 1.2)]
        ...
        for pos in candidates:
            ...
            if h < lowground_h:
                continue
            in_ramp_range = float(pos.distance_to(self.hold_pos)) <= siege_range
            if not in_ramp_range:
                continue
            score = 0.0
            score += 8.0 if h > lowground_h else 2.0
            score += max(0.0, 4.0 - float(pos.distance_to(preferred_center)))
            if float(pos.distance_to(self.base_pos)) <= siege_range:
                score += 3.0
            if score > best_score:
                best = pos
                best_score = score
    ...
    def _handle_general(self, *, unit, slot: Point2, enemy_near: list, bunkers: list) -> bool:
    ...
    def _handle_scv(self, *, unit, slot: Point2, enemy_near: list, repair_targets: list) -> bool:
    ...
    def _handle_probe_unit(self, *, unit, enemy_near: list) -> bool:
        if enemy_near:
    ...
    async def on_step(self, bot, tick: TaskTick, attention: Attention) -> TaskResult:
        ...
        front_tank_anchor = self._safe_anchor(
            bot,
            preferred=self.hold_pos,
        ...
        for unit in units:
            ...
            if is_probe and not lowground_cleared:
                issued = self._handle_probe_unit(unit=unit, enemy_near=enemy_near) or issued
                continue
            ...
            if unit.type_id == U.SCV:
                ...
                scv_idx += 1
                issued = self._handle_scv(unit=unit, slot=slot, enemy_near=enemy_near, repair_targets=repair_targets) or issued
                continue
            if not lowground_cleared:
            ...
            general_idx += 1
            issued = self._handle_general(unit=unit, slot=slot, enemy_near=enemy_near, bunkers=bunkers) or issued
