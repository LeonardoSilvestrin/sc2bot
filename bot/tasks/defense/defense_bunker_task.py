...
from bot.intel.utils.defensive_placements import point_from_payload
...
class DefenseBunkerTask(BaseTask):
    ...
    def _reserve_worker(bot, worker) -> None:
        try:
            # BUILDING before entering building_tracker makes Ares snap the SCV
            # back to GATHERING and issue a mineral gather order.
    ...
    def _intel_main_ramp_bunker_pos(self, *, now: float) -> Point2 | None:
        ...
        return point_from_payload(payload)

    def _intel_nat_bunker_pos(self, *, now: float, base_pos: Point2) -> Point2 | None:
        if self.awareness is None:
            return None
        pos = point_from_payload(self.awareness.mem.get(K("intel", "placements", "nat_bunker"), now=now, default=None))
        if pos is None:
            return None
        try:
            if float(pos.distance_to(base_pos)) > 12.0:
                return None
        except Exception:
            return None
        return pos

    def _intel_perimeter_bunker_pos(self, *, now: float, base_pos: Point2) -> Point2 | None:
        if self.awareness is None:
            return None
        payload = self.awareness.mem.get(K("intel", "placements", "outer_bunkers"), now=now, default=[])
        ...
        for entry in payload:
            if not isinstance(entry, dict):
                continue
            ref_base = point_from_payload(entry.get("base"))
            pos = point_from_payload(entry.get("pos"))
            if ref_base is None or pos is None:
                continue
            try:
                dist = float(ref_base.distance_to(base_pos))
            except Exception:
                continue
            if dist > 6.5:
                continue
            if best is None or float(dist) < float(best[0]):
                best = (float(dist), pos)
    ...
    async def _choose_bunker_position(self, bot, *, base_pos: Point2, anchor: Point2, now: float) -> Point2 | None:
        ...
        if mode == "MAIN_RAMP":
            ...
            return None
        if mode == "NAT_CHOKE":
            intel_nat_pos = self._intel_nat_bunker_pos(now=now, base_pos=base_pos)
            if intel_nat_pos is not None and not self._is_blocked_position(intel_nat_pos):
                try:
                    if bool(bot.mediator.can_place_structure(position=intel_nat_pos, structure_type=U.BUNKER)):
                        return intel_nat_pos
                except Exception:
                    pass
            try:
                pos = await bot.find_placement(U.BUNKER, near=anchor, max_distance=6)
                ...
                pass
        if mode == "PERIMETER":
            intel_outer_pos = self._intel_perimeter_bunker_pos(now=now, base_pos=base_pos)
            if intel_outer_pos is not None and not self._is_blocked_position(intel_outer_pos):
                try:
                    if bool(bot.mediator.can_place_structure(position=intel_outer_pos, structure_type=U.BUNKER)):
                        return intel_outer_pos
                except Exception:
                    pass
        try:
            pos = bot.mediator.request_building_placement(
