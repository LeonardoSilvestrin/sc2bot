...
class DefenseBunkerTask(BaseTask):
    ...
    def _bunker_anchor(self, bot, *, base_pos: Point2) -> Point2:
        ...
        if mode == "NAT_CHOKE":
            try:
                return base_pos.towards(bot.start_location, 4.5)
            except Exception:
                return base_pos
        if mode == "PERIMETER":
            try:
    ...
    def _bunker_started_or_ready(self, bot, *, base_pos: Point2) -> bool:
        ...
        ref_radius = 9.0 if use_anchor else 16.0
        try:
    ...
    async def _choose_bunker_position(self, bot, *, base_pos: Point2, anchor: Point2) -> Point2 | None:
        mode = str(self.anchor_mode or "BASE").upper()
        if mode in {"MAIN_RAMP", "NAT_CHOKE"}:
            try:
        ...
        try:
            ...
            pass
        try:
            return await bot.find_placement(U.BUNKER, near=anchor, max_distance=8)
        except Exception:
            return None
    ...
    async def on_step(self, bot, tick: TaskTick, attention: Attention) -> TaskResult:
        ...
        anchor = self._bunker_anchor(bot, base_pos=base_pos)
        pos = await self._choose_bunker_position(bot, base_pos=base_pos, anchor=anchor)
        if pos is None:
            return TaskResult.noop("bunker_position_unavailable")
        try:
            worker = bot.mediator.select_worker(target_position=pos, force_close=True)
        except Exception:
            worker = None
        if worker is None:
            return TaskResult.noop("bunker_worker_unavailable")
        try:
            if bool(
            ...
            return TaskResult.noop("bunker_build_command_failed")
        return TaskResult.noop("bunker_build_command_rejected")
