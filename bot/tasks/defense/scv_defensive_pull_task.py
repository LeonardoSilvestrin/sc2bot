    ...
    async def on_step(self, bot, tick: TaskTick, attention: Attention) -> TaskResult:
        ...
        priority_targets = sorted(
            ...
            key=lambda e: (
                ...
                float(base_pos.distance_to(e)),
                self._enemy_priority(e)[1],
            ),
        )
        issued = False
        fallback_hold = threat.towards(base_pos, 3.0) if threat != base_pos else base_pos
        for scv in units:
            close_enemy = enemy_near.closer_than(5.5, scv.position)
            if close_enemy.amount > 0:
                target = min(
                    list(close_enemy),
                    key=lambda e: (
                        -self._enemy_priority(e)[0],
                ...
                continue
            if priority_targets:
                scv.attack(priority_targets[0])
                issued = True
                continue
            if float(scv.distance_to(fallback_hold)) > 2.0:
                scv.move(fallback_hold)
                issued = True
