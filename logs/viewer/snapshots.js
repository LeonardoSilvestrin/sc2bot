// SVG field snapshots: when each one was taken and which one belongs to a
// selected instant. Pure: files are opaque handles supplied by the page.
(function (root) {
  "use strict";

  // field-0246.svg -> 246, field-0246-500.svg -> 246.5 (territory-* is legacy)
  const SNAPSHOT_NAME = /(?:field|territory)-(\d+)(?:-(\d{1,3}))?\.svg$/i;
  const SNAPSHOT_EVENTS = new Set(["logs.snapshot_written", "debug.spatial_snapshot_written"]);
  ...
  function buildSnapshotIndex(records, files = []) {
    ...
    for (const record of records) {
      if (!SNAPSHOT_EVENTS.has(record.event)) continue;
      const name = baseName(record.data?.path);
      if (!name) continue;
      byName.set(name, { t: record.game_time, name, file: null, logged: true, trigger: record.data?.trigger });
    }
